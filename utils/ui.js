import { amortiguador } from "./utils";

export function adicionarPerspectivaMarco() {
  const elementoMarco = document.querySelector("#marco-ventana");

  let rotacionXNorm = 0;
  let rotacionYNorm = 0;

  let altoVentana = window.innerHeight;
  let anchoVentana = window.innerWidth;

  const cambioPerspectiva = (e) => {
    rotacionXNorm = ((e.clientY / altoVentana - 0.5) * 9).toFixed(2);
    rotacionYNorm = ((e.clientX / anchoVentana - 0.5) * -9).toFixed(2);
    elementoMarco.style.setProperty("--rotacion-x", `${rotacionXNorm}deg`);
    elementoMarco.style.setProperty("--rotacion-y", `${rotacionYNorm}deg`);
  };

  const cambioCentrar = () => {
    setTimeout(() => {}, 500);
    elementoMarco.style.setProperty("--rotacion-x", `0deg`);
    elementoMarco.style.setProperty("--rotacion-y", `0deg`);
  };

  const cambioPerspectivaAmortiguado = amortiguador(cambioPerspectiva, 100);

  document.addEventListener("pointerover", cambioPerspectivaAmortiguado);

  window.addEventListener("resize", () => {
    altoVentana = window.innerHeight;
    anchoVentana = window.innerWidth;
  });
}
